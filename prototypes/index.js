...
const kittyPrompts = {
  ...
  growUp() {
    ...
    const result = kitties.map(obj => {
        newAgeObject = obj
        newAgeObject.age = obj.age += 2;
        return newAgeObject;
    });
    return result;
  }
...
const clubPrompts = {
  membersBelongingToClubs() {
    ...
    const result = clubs.reduce((membersInClubs, club) => {
  const clubName = club.club;
  club.members.forEach(member => {
    if(!membersInClubs[member]) {
      membersInClubs[member] = [];
    }
    membersInClubs[member].push(clubName)
  }) 
  return membersInClubs;
},{});;
    return result;
...
const breweryPrompts = {
  getBeerCount() {
    ...
    const result =  breweries.reduce((acc, curr) => {
  curr.beers.forEach(beer => {
    acc ++;
  })
  return acc;
},0)
